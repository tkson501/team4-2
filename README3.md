<img title="" src="images/logo.jpg" alt="images" width="295">
...
1. 고객이 꽃을 선택하여 주문한다
2. 고객이 결제한다
3. 결제가 되면 주문 내역이 입점상점주인에게 전달된다
4. 배달(Online)으로 주문한 경우 택배사로 전달된다.
5. 직접수령(Offline)으로 주문한 경우 배달완료 처리한다.
6. 고객이 주문을 취소할 수 있다
7. 주문이 취소되면(결제취소 후) 배달이 취소된다
8. 고객이 주문상태를 중간중간 조회한다
9. 주문상태가 바뀔 때 마다 카톡으로 알림을 보낸다 (미구현)
...
3. 성능
   ...
   2. 배달상태가 바뀔때마다 카톡 등으로 알림을 줄 수 있어야 한다  Event driven (미구현)
...
### 어그레게이트 배치하고, 바운디드 컨텍스트로 묶어보기

![](images/0.es-2.jpg)

### 세부 구성 (Pub/Sub, Req/Res 설정시작)
